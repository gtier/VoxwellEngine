...
{
    // Gets size of indices that say how to render vertices
...
void mes::MaxwellEngine::queueRenderObjectCreation(mes::VertexDataObject<float>& verts, std::vector<unsigned int>& indices, glm::mat4& translation)
{
    RenderObjectData ro_data;
    ro_data.verts = std::move(verts);
    ro_data.indices = std::move(indices);
    ro_data.translation = std::move(translation);
    
    queueRenderObject.enqueue(std::move(ro_data));
}
...
//        }
        ...
        // Check render object queue
        // If not empty, create new render object
        // Else, continue
        
        while (!queueRenderObject.empty()) {
            RenderObjectData ro_data;
            queueRenderObject.mv_dequeue(ro_data);
            obj_id id = createRenderObject(ro_data.verts, ro_data.indices);
            vecRenderObject[id]->setModelMatrix(ro_data.translation);
        }
...
// Mounts a CameraObject to the staticCameraPtr
