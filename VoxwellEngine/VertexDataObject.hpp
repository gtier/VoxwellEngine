...
public:
    ...
    VertexDataObject& operator=(VertexDataObject&& other)
    {
        vertexDataPtr = std::move(other.vertexDataPtr);
        vertexTypes = std::move(other.vertexTypes);
        return *this;
    }
...
private:
    int getGLenum() {
        if (std::is_same<T, float>::value) {
            ...
            throw VoxwellError("VertexDataObject getGLenum: invalid VertexDataObject type. Must be float, int, or double");
        }
    }
};
